...
public class K8sUnicastHostsProvider extends AbstractComponent implements
  ...
  public K8sUnicastHostsProvider(Settings settings,
    ...
    this.refreshInterval = componentSettings.getAsTime(Fields.REFRESH,
        settings.getAsTime("cloud.k8s." + Fields.REFRESH, TimeValue.
            timeValueSeconds(0)));

    this.selector = componentSettings.get(Fields.SELECTOR, settings.get(
        "cloud.k8s." + Fields.SELECTOR));
    this.serviceDns = componentSettings.get(Fields.SELECTOR, settings.get(
        "cloud.k8s." + Fields.SERVICE_DNS));

    // Check that we have all needed properties
    if (!(Strings.hasText(this.selector) || Strings.hasText(this.serviceDns))) {
      logger.warn("Neither cloud.k8s.{} or cloud.k8s.{} are set.",
          Fields.SELECTOR, Fields.SERVICE_DNS);
    }
  }
  ...
  public List<DiscoveryNode> buildDynamicNodes() {
    if (refreshInterval.millis() != 0) {
      if (cachedDiscoNodes != null &&
          (refreshInterval.millis() < 0 || (System.currentTimeMillis() - lastRefresh) < refreshInterval.
          millis())) {
    ...
    try {
      ...
      logger.warn("Exception caught during discovery {} : {}", e, e.getClass().
  ...
  private List<DiscoveryNode> getNodesFromKubernetesSelector(
      String currentIpAddress) {
    // get all endpoints for service
    final Endpoints endpoints = getKubernetes().endpointsForService(
        this.selector, null);

    if (endpoints.getEndpoints().isEmpty()) {
      logger.trace("No pods found for selector [{}].", this.selector);
      return cachedDiscoNodes;
    }

    // populate discovery nodes list
    for (String endpoint : endpoints.getEndpoints()) {
      logger.trace("Handling endpoint {}", endpoint);
      final String[] splitEndpoint = endpoint.split(":");
      final String endpointIp = splitEndpoint[0];
      final String endpointPort = splitEndpoint[1];
      if (endpointIp.equals(currentIpAddress)) {
        ...
        // connect only to those that expose Elasticsearch Transport port, 9300
        if (!endpointPort.equals("9300")) {
          logger.trace(
              "Endpoint does not expose Elasticsearch transport port. Ignoring {}",
              endpointIp);
        } else {
          // pod IP is a single IP Address. We need to build a TransportAddress from it
          final TransportAddress[] addresses;
          try {
            addresses = transportService.addressesFromString(endpoint);
            logger.trace("Adding address {}", addresses[0]);
            cachedDiscoNodes.add(new DiscoveryNode(endpointIp, addresses[0],
                Version.CURRENT));
          } catch (Exception ex) {
  ...
  private List<DiscoveryNode> getNodesFromKubernetesServiceDns(
      String currentIpAddress) throws Exception {
    Set<String> serviceEndpointIps = KubernetesHelper.lookupServiceInDns(
        serviceDns);

    if (serviceEndpointIps == null) {
      logger.trace("no service endpoints found for service name [{}].",
          this.serviceDns);
      return cachedDiscoNodes;
    }
    ...
    for (String endpointIp : serviceEndpointIps) {
      if (endpointIp.equals(currentIpAddress)) {
        ...
        // pod IP is a single IP Address. We need to build a TransportAddress from it
        TransportAddress[] addresses = transportService.addressesFromString(
            address);
        ...
        cachedDiscoNodes.add(new DiscoveryNode("#cloud-".concat(serviceDns).
            concat("-").concat(endpointIp) + "-" + 0, addresses[0],
            Version.CURRENT));
      }
    }
